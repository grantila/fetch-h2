...
describe('nghttp2.org/httpbin', function () {
    ...
    it('should be possible to GET HTTPS/2', async () => {
        const response = await __1.fetch('https://nghttp2.org/httpbin/user-agent');
        const data = await response.json();
        chai_1.expect(data['user-agent']).to.include('fetch-h2/');
    });
    it('should be possible to POST JSON', async () => {
        const testData = { foo: 'bar' };
        const response = await __1.fetch('https://nghttp2.org/httpbin/post', {
            method: 'POST',
    ...
    it('should be possible to POST not yet ended stream-data', async () => {
        const stream = through2();
        const eventual_response = __1.fetch('https://nghttp2.org/httpbin/post', {
            method: 'POST',
            body: new __1.StreamBody(stream),
            headers: { 'content-length': '6' },
        });
